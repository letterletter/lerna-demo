 ...
 * 使用app/routes中的文件系统约定定义路由，规则如下：
 * 路由路径派生自文件路径。文件名中的‘.’表示URL中的‘/’（嵌套URL，而不是路由嵌套），一个‘$’在文件名中表示动态URL段
 */
export function defineConventionalRoutes(
  ...
  function defineNestedRoutes(
    ...
    console.log('parentId', parentId, childRouteIds)
    for (let routeId of childRouteIds) {
      ...
      console.log('fullpath', fullPath, uniqueRouteId, isIndexRoute)
      if (uniqueRouteId) {
