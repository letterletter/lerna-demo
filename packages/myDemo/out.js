    ...
    function patch(fs2) {
      ...
      function patchLchmod(fs3) {
        fs3.lchmod = function(path5, mode, callback) {
          fs3.open(path5, constants.O_WRONLY | constants.O_SYMLINK, mode, function(err, fd) {
            if (err) {
              if (callback)
                callback(err);
              return;
...
function defineRoutes(callback) {
  ...
  let defineRoute = (path5, file, optionsOrChildren, children) => {
    if (alreadyReturned) {
...
function isRouteModuleFile(filename) {
  return routeModuleExts.includes(path2.extname(filename));
}
function defineConventionalRoutes(appDir, ignoredFilePatterns) {
  let files = {};
  visitFiles(path2.join(appDir, "routes"), (file) => {
    if (ignoredFilePatterns && ignoredFilePatterns.some((pattern) => (0, import_minimatch.default)(file, pattern))) {
      return;
    }
    if (isRouteModuleFile(file)) {
      let routeId = createRouteId(path2.join("routes", file));
      files[routeId] = path2.join("routes", file);
      return;
    }
    throw new Error(`Invalid route module file: ${path2.join(appDir, "routes", file)}`);
  });
...
function createRoutePath(partialRouteId) {
  ...
  for (let i = 0; i < partialRouteId.length; i++) {
    ...
    let lastChar = i > 0 ? partialRouteId.charAt(i - 1) : void 0;
    let nextChar = i < partialRouteId.length - 1 ? partialRouteId.charAt(i + 1) : void 0;
    if (skipSegment) {
      if (char === "/" || char === "." || char === path2.win32.sep) {
        skipSegment = false;
      }
      continue;
    }
    ...
    if (char === "/" || char === path2.win32.sep || char === ".") {
...
function visitFiles(dir, visitor, baseDir = dir) {
  for (let filename of fs.readdirSync(dir)) {
    let file = path2.resolve(dir, filename);
    let stat = fs.lstatSync(file);
    if (stat.isDirectory()) {
      visitFiles(file, visitor, baseDir);
    } else if (stat.isFile()) {
      visitor(path2.relative(baseDir, file));
    }
  }
}

// config/serverModes.ts
function isValidServerMode(mode) {
  return mode === "development" /* Development */ || mode === "production" /* Production */ || mode === "test" /* Test */;
}

// compiler/virtualModules.ts
var serverBuildVirtualModule = {
  id: "@remix-run/dev/server-build",
  filter: /^@remix-run\/dev\/server-build$/
};

// config.ts
async function readConfig(remixRoot, serverMode = "production" /* Production */) {
  if (!remixRoot) {
  ...
  let rootDirectory = path3.resolve(remixRoot);
  let configFile = path3.resolve(rootDirectory, "remix.config.js");
...
${String(error)}`);
  ...
  let appDirectory = path3.resolve(rootDirectory, appConfig.appDirectory || "app");
  let cacheDirectory = path3.resolve(rootDirectory, appConfig.cacheDirectory || ".cache");
  ...
  if (fse.existsSync(path3.resolve(appDirectory, "routes"))) {
    let conventionalRoutes = defineConventionalRoutes(appDirectory, appConfig.ignoredRouteFiles);
  ...
  let serverBuildTargetEntryModule = `export * from ${JSON.stringify(serverBuildVirtualModule.id)};`;
...
function addTrailingSlash(path5) {
  return path5.endsWith("/") ? path5 : path5 + "/";
}
var entryExts = [".js", ".jsx", ".ts", ".tsx"];
function findEntry(dir, basename) {
  for (let ext of entryExts) {
    let file = path3.resolve(dir, basename + ext);
    if (fse.existsSync(file))
      return path3.relative(dir, file);
  }
  return void 0;
}
...
console.log(import_path.default.resolve(__dirname));
readConfig(import_path.default.resolve(__dirname));
